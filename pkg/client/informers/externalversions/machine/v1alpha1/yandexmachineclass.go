...
type YandexMachineClassInformer interface {
	...
	Lister() v1alpha1.YandexMachineClassLister
}
...
func NewYandexMachineClassInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredYandexMachineClassInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.MachineV1alpha1().YandexMachineClasses(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.MachineV1alpha1().YandexMachineClasses(namespace).Watch(options)
			},
		},
		&machinev1alpha1.YandexMachineClass{},
		resyncPeriod,
		indexers,
	)
}

func (f *yandexMachineClassInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *yandexMachineClassInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&machinev1alpha1.YandexMachineClass{}, f.defaultInformer)
}

func (f *yandexMachineClassInformer) Lister() v1alpha1.YandexMachineClassLister {
	return v1alpha1.NewYandexMachineClassLister(f.Informer().GetIndexer())
}
